    ...
    return (
        ...
        >
            <div className="space-y-12 mx-auto px-4 max-w-6xl container">
                ...
                <div className="gap-8 lg:gap-16 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 pb-2">
                    {items?.map((item, index) => (
                        <FadeUp delay={0.1 * index} duration={0.3 * index} key={index} className="group relative flex flex-col justify-center space-y-4 p-8 sm:p-12 lg:p-16 border hover:border-secondary-foreground/60 border-transparent rounded-4xl text-center transition-all duration-500 ease-in-out transform">
                            <img src={item?.icon?.file} alt={item?.title} className="mx-auto" />
                            <h3 className="font-light text-secondary-foreground text-3xl lg:text-4xl">{item?.title}</h3>
